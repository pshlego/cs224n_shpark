...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        transition_list = model.predict(mini_batch)
        for partial_p, transition in zip(mini_batch, transition_list):
            partial_p.parse_step(transition)
            if len(partial_p.stack)==1 and len(partial_p.buffer)==0:
                unfinished_parses.remove(partial_p)
    dependencies = [partial_p.dependencies for partial_p in partial_parses] 
