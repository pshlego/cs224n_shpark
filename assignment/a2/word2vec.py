...
def naiveSoftmaxLossAndGradient(
    ...
    loss = -np.log(softmax(np.matmul(outsideVectors,np.transpose(centerWordVec)))[outsideWordIdx])
    one_hot = np.zeros(outsideVectors.shape[0])
    one_hot[outsideWordIdx]=1
    gradCenterVec = np.matmul(softmax(np.matmul(outsideVectors,np.transpose(centerWordVec))) - one_hot, outsideVectors)
    gradOutsideVecs = np.outer(softmax(np.dot(outsideVectors,centerWordVec)) - one_hot, centerWordVec)
    ### Please use the provided softmax function (imported earlier in this file)
    ### This numerically stable implementation helps you avoid issues pertaining
    ### to integer overflow. naiveSoftmaxLossAndGradient(centerVec, 1, vec, dataset)
...
def negSamplingLossAndGradient(
    ...
    s = np.dot(outsideVectors[outsideWordIdx],centerWordVec)
    loss = -np.log(sigmoid(s))
    loss += -np.sum(np.log(sigmoid(-np.dot(outsideVectors,centerWordVec)))[negSampleWordIndices])
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradcontextvec= (sigmoid(s)-1)*centerWordVec
    gradOutsideVecs[outsideWordIdx] = gradcontextvec
    gradCenterVec = (sigmoid(s)-1)*outsideVectors[outsideWordIdx]
    for i in negSampleWordIndices:
        sc = np.dot(outsideVectors[i],centerWordVec)
        gradCenterVec += (1-sigmoid(-sc))*outsideVectors[i]
        gradOutsideVecs[i] += (1-sigmoid(-sc))*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerword = centerWordVectors[word2Ind[currentCenterWord]]
    for context_st in outsideWords:
        context_id = word2Ind[context_st]
        ls, gradCV, gradOV = word2vecLossAndGradient(centerword,context_id,outsideVectors,dataset)
        loss+=ls
        gradCenterVecs[word2Ind[currentCenterWord]]+=gradCV
        gradOutsideVectors+=gradOV
    ### END YOUR CODE
