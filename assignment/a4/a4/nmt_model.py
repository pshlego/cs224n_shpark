...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = pack_padded_sequence(X,source_lengths)# (src_len*b,e)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0].permute(1, 0, 2)
        h_fwd, h_bwd = last_hidden[0], last_hidden[1]
        h_enc_cat = torch.cat([h_fwd, h_bwd], dim=1)
        init_decoder_hidden = self.h_projection(h_enc_cat)#W_h[h_enc,h_enc]

        c_fwd, c_bwd = last_cell[0], last_cell[1]
        c_enc_cat = torch.cat([c_fwd, c_bwd], dim=1)
        init_decoder_cell = self.c_projection(c_enc_cat)#W_c[c_enc,c_enc]
        
        dec_init_state = (init_decoder_hidden, init_decoder_cell)#tuple
        ### END YOUR CODE
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t=torch.einsum('bi,bji->bj', dec_hidden, enc_hiddens_proj)
        ...
        a_t = torch.einsum('bi,bij->bj', alpha_t, enc_hiddens)
