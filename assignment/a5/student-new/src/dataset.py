...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc_length=random.randint(4,int(self.block_size*7/8))#4~int(self.block_size*7/8)
        truncate_length = min(document_length, trunc_length)
        truncated_document = document[:truncate_length]
        # number 2
        range_mask = int(1 / 8 * truncate_length)
        masked_length = random.randint(int(1 / 4 * truncate_length)-(range_mask), int(1 / 4 * truncate_length)+(range_mask))
        ...
        # number 3
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * (self.block_size - truncate_length - 2)
        # number 4
        x = masked_string[:-1]
        y = masked_string[1:]
        # number 5
